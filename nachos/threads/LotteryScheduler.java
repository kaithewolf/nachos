...
public class LotteryScheduler extends Scheduler{
    ...
    public int getPriority(KThread thread) {
		...
		setPriority(thread, tickets+1); //add 1 ticket
    	...
    	setPriority(thread, tickets-1); // -1 ticket
    	...
    	public KThread nextThread() {
    	    ...
    	    if(this.waitingQueue.isEmpty()) {
    	    	...
    	    	this.owner = (KThread)waitingQueue.poll();
    	    }
    			...
    			for(int i = 0; i <= this.queue.lastIndexOf(this.queue.getLast()); i++) {
    				...
    				if(this.queue.get(i).transferPriority && !this.queue.get(i).waitingQueue.isEmpty()) {
    					//System.out.print(this.queue.get(i)+"->");
    						total += this.queue.get(i).chooseThread(i).getEffectivePriority(i+1);
    	...
    	/** Lottery Queue's list of threads**/
    	protected LinkedList<LotteryQueue> queue;
		
		/**total number of tickets for this */
